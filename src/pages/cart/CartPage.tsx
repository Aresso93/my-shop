import { useCart, selectCartList, selectTotalCartCost, selectCartIsEmpty } from "@/services/cart"
            ...
            <ul>
                {
                    list.map(p => (
                            <li 
                            key={p.product.id}
                            className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-blue-400 my-3"
                            >
                                <div className="flex items-center gap-3">
                                    <img src={p.product.tmb} alt={p.product.name} className="w-24 rounded-xl" />
                                    <div className="font-bold">{p.product.name} x {p.qty}</div>
                                </div>
